 ...
 *   Secure    — HTTPS only (set whenever the request arrived over HTTPS, so
 *               it applies behind Render's TLS proxy but not on localhost)
 *   Path=/    — one cookie for the whole app, no Domain (host-only)
 */
function cookieFlags(isSecure) {
  const secure = isSecure || process.env.NODE_ENV === 'production' ? '; Secure' : '';
  return `Path=/; HttpOnly; SameSite=Lax${secure}`;
}

function setSessionCookie(res, token, isSecure = false) {
  res.setHeader('Set-Cookie',
    `${SESSION_COOKIE}=${token}; ${cookieFlags(isSecure)}; Max-Age=${SESSION_DAYS * 86400}`);
}

function clearSessionCookie(res, isSecure = false) {
  res.setHeader('Set-Cookie', `${SESSION_COOKIE}=; ${cookieFlags(isSecure)}; Max-Age=0`);
}
...
function signup(email, password) {
  ...
  if (!EMAIL_RE.test(email)) throw inputError('Please enter a valid email address.');
  if (password.length < 8) throw inputError('Password must be at least 8 characters.');
  if (password.length > 200) throw inputError('Password is too long.');

  const exists = db.prepare('SELECT id FROM users WHERE email = ?').get(email);
  if (exists) throw inputError('An account with that email already exists. Try signing in.');
...
function login(email, password) {
  ...
  if (!ok) throw inputError('Incorrect email or password.', 401);
