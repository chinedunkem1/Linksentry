...
function userForSession(token) {
  ...
  return db.prepare(
    `SELECT u.id, u.email, u.plan, u.stripe_customer_id, u.created_at
...
function attachUser(req, _res, next) {
  ...
  if (req.user && isProEmail(req.user.email) && req.user.plan !== 'pro') {
...
function signup(email, password) {
  ...
  const plan = isProEmail(email) ? 'pro' : 'free';
  const info = db.prepare('INSERT INTO users (email, password_hash, plan) VALUES (?, ?, ?)')
    .run(email, hashPassword(password), plan);
  return { id: info.lastInsertRowid, email, plan };
}
