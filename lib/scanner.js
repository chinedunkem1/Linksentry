...
function normalizeInput(raw) {
  ...
  if (!input) throw new Error('Please enter a URL to scan.');
  if (input.length > 2048) throw new Error('URL is too long (max 2048 characters).');
  if (/\s/.test(input)) throw new Error('URLs cannot contain spaces. Please check your input.');
...
function fetchWithTimeout(url, options = {}, ms = 8000) {
  return fetch(url, {
    ...
    headers: {
      'User-Agent': 'Mozilla/5.0 (compatible; LinkSentryBot/1.0; +https://linksentry.app)',
      'Accept': '*/*',
      ...(options.headers || {}),
    },
  });
...
function staticChecks(url, findings) {
  ...
  if (SHORTENERS.has(regDomain)) {
    findings.push({
      ...
      detail: `${regDomain} hides the real destination. LinkSentry will follow the redirect chain to reveal where it actually leads.`,
    });
  }
  ...
  try {
    const res = await fetchWithTimeout(
      `https://safebrowsing.googleapis.com/v4/threatMatches:find?key=${key}`,
      {
        ...
        body: JSON.stringify({
          client: { clientId: 'linksentry', clientVersion: '1.0.0' },
          threatInfo: {
...
async function scanUrl(rawInput) {
  ...
  try {
    ...
    throw new Error('That does not look like a valid URL. Example: https://example.com/page');
  }
  if (!['http:', 'https:', 'javascript:', 'data:', 'vbscript:'].includes(url.protocol)) {
    throw new Error(`Unsupported scheme "${url.protocol}" — LinkSentry scans web links (http/https).`);
  }
